...
def softmax_net(x, w, b):
    return softmax(linear_layer(x, w, b))
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for batchx, batchy in test_loader:
                right_count += correct_count(softmax_net(batchx, w, b), batchy)
            accuracy_train = right_count_train / total_samples
            accuracy = right_count / len(test_loader.dataset)
            avg_loss = total_loss / total_samples
            print(f"Epoch {epoch + 1}/{epochs} | Train Loss: {avg_loss:.4f} | Train Acc :{accuracy_train:.4f} Test Acc: {accuracy:.4f}")
